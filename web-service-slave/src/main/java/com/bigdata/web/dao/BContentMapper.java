...
public interface BContentMapper {
	...
	@Select("select `cid`, `title`, `slug`, `created`, `modified`, `content`, `type`, `tags`, `categories`, `hits`, `comments_num`, `allow_comment`, `allow_ping`, `allow_feed`, `status`, `author`, `image_url`, `link`, `publish_date` from b_content where cid = #{id}")
	...
	@Select("<script>" +
			...
			"<choose>" +
				...
				"</when>" +
				"<otherwise>" +
					"and image_url is not null and allow_feed = 0 " +
				"</otherwise>" +
			"</choose>" +
			"<if test=\"type != null and type != ''\">"+ "and type = #{type} " + "</if>" +
			"</where>"+
			"order by cid desc limit #{num}" +
			"</script>")
	...
	@Insert("insert into b_content (`title`, `slug`, `created`, `modified`, `content`, `type`, `tags`, `categories`, `hits`, `comments_num`, `allow_comment`, `allow_ping`, `allow_feed`, `status`, `author`, `image_url`, `link`, `publish_date`)"
	+ "values (#{title}, #{slug}, #{created}, #{modified}, #{content}, #{type}, #{tags}, #{categories}, #{hits}, #{commentsNum}, #{allowComment}, #{allowPing}, #{allowFeed}, #{status}, #{author}, #{imageUrl}, #{link}, #{publishDate})")
